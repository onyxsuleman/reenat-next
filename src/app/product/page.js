...
// Handles both new schema columns and old cached data gracefully.
...
export async function generateMetadata({ searchParams }) {
  ...
  if (!idParam) return { title: 'Product Not Found | Reenat Trends' };
...
export default async function ProductDetailsPage({ searchParams }) {
  ...
  // 1. Fetch the requested product by product_id (text PK, e.g. NSY9M1001)
  ...
  const catalogCode = product.catalog_code || '';

  // 2. Fetch all products in the same catalog group (color variants).
  //    Fall back to just the current product if no catalog_code is set.
  ...
  if (catalogCode) {
    ...
    if (variantRows && variantRows.length > 0) {
      // De-duplicate by product_id (safety guard)
      const seen = new Set();
  ...
  try {
    ...
    if (allProducts && allProducts.length > 0) {
      ...
      if (catalogCode) seenCatalogs.add(catalogCode.toLowerCase());

      recommended = allProducts
        .map(mapRawProduct)
        .filter(p => {
          if (String(p.product_id) === String(product.product_id)) return false;
          const cid = p.catalog_code ? p.catalog_code.toLowerCase() : '';
