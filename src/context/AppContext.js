...
const defaultProducts = [
  ...
  },
  {
    ...
    "origin": "India",
    "craft": null,
    "desc_text": "Make a majestic entrance with our newly launched Hathi Raja design. Inspired by the royal elephants of Indian heritage—a timeless symbol of prosperity, strength, and grandeur—this stunning new arrival brings a touch of regal elegance to your traditional wardrobe.\n\nCrafted for the modern woman who values her roots, this saree effortlessly combines heritage motifs with everyday comfort. The magnificent elephant-inspired weaving makes it a true statement piece, guaranteed to turn heads at your next big event.",
    "gst": "5",
    "hsn": "520811",
    "weight": 450,
    "sku": "M8||Gold Black HR1",
    "blouselen": "0.8",
    "sareelen": "5.5",
    "blousetype": "Zari Woven",
    "blousecolor": "Gold",
...
const mapRawProduct = (raw) => {
  // Resolve the canonical product_id: new schema uses product_id (text PK),
  // old cached data may have numeric id or legacy productId format
  const productId = raw.product_id || raw.productId || '';
  const legacyId = raw.legacy_id || raw.id || null;

  return {
    // New schema primary key
    product_id: productId,
    productId: productId,
    // Legacy numeric id (for backward compat with old cached cart/wishlist)
    id: productId || legacyId,
    legacy_id: legacyId,
    name: raw.name,
    type: raw.type,
    color: raw.color,
    price: Number(raw.price) || 0,
    // New column: mrp (was originalprice)
    originalPrice: Number(raw.mrp || raw.originalprice || raw.originalPrice) || 0,
    mrp: Number(raw.mrp || raw.originalprice || raw.originalPrice) || 0,
    ...
    // New column: desc_text (was desc — SQL reserved word)
    desc: raw.desc_text || raw.desc || '',
    desc_text: raw.desc_text || raw.desc || '',
    gst: raw.gst || '5',
    hsn: raw.hsn || '520811',
    weight: raw.weight || 450,
    // New column: sku (was styleid)
    styleId: raw.sku || raw.styleid || raw.styleId || '',
    styleid: raw.sku || raw.styleid || raw.styleId || '',
    sku: raw.sku || raw.styleid || raw.styleId || '',
    ...
    // New column: border_type (was border — SQL reserved word)
    border: raw.border_type || raw.border || 'Zari',
    border_type: raw.border_type || raw.border || 'Zari',
    ...
    // New column: catalog_code (was catalog_id)
    catalogId: raw.catalog_code || raw.catalog_id || raw.catalogId || '',
    catalog_id: raw.catalog_code || raw.catalog_id || raw.catalogId || '',
    catalog_code: raw.catalog_code || raw.catalog_id || raw.catalogId || '',
    skuId: raw.sku || raw.skuId || raw.styleid || raw.styleId || '',
